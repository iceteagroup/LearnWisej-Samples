...
- **Server stays the authority.** The id is re-loaded, the rule is the domain's, the URL is built and
  signed server-side, the audit entry is written only after the browser confirmed. An unknown id, a
  confidential work order and a denied clipboard each stop at the right layer with a plain sentence.
- **Nothing leaked.** `Strings.*` in the banner; exception details and the browser's error text go to the
  server log only.
...
| Log WebMethod refusals | `ReportShortcut` already refuses unknown names; a production log should alert on repeated refusals from one session (a sign of tampering) | `Controls/GlobalSearchBox.cs` |
...
| Accessibility of the `?` list | it is a plain `div` with `role="dialog"`; add focus management if it grows beyond a hint | `Platform/ticketops.interop.js` |
...
`dotnet run -f net10.0 --urls http://localhost:5109`, then press Ctrl+K, search, select a work order and
click **Copy link**: the status confirms the copy and the audit log gains a line.
