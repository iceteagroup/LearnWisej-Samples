    ...
    public partial class ActivityTracePanel : UserControl
    {
        ...
        public static string Format(ActivityEntry entry)
        {
            ...
            string detail = entry.Exception != null
                ...
                : entry.Message;
            // Module 9 (escaping): the log carries user-typed text (the search query) and browser-reported text (the
            // clipboard error). The ListBox escapes item text itself (verified: an encoded "&#183;" shows up literally),
            // so the line is handed over as plain text — encoding here would double-escape it.
