...
Open <http://localhost:5102> in a **second browser tab**. Each tab is its own Wisej.NET session — like a
second launch of a desktop app. Call them **A** and **B**.
...
| # | Deliverable | Where |
|---|---|---|
| 1 | Project runs locally without missing references | `dotnet build -nologo -v q` — 0 errors |
...
| 3 | Registered per session (session lifetime) | `Infrastructure/AppComposition.cs` — one composition per `Program.Main` call, i.e. per session |
...
| 6 | Shows session id, user, tenant, theme, client profile | the green panel rows |
...
- **Which fields would be unsafe if static?**
  ...
  `SessionContext`. Safe statics are the ones left in the project: immutable `AppSettings`, the
  lock-protected `SharedTicketStore`, the `Interlocked` counter, pure functions and constants.
- **What should happen when a browser refreshes?**
  ...
  and the services are kept — nothing is recomposed. A new tab, on the other hand, is a new session with a
  fresh composition. Durable data survives both because it lives in the repository, not in the session.
...
- **Which startup code runs once per process versus once per session?**
  `Startup.cs` and `ProcessScope.LoadSettings` once per process; `Program.Main` → `AppComposition` once per
  session.
