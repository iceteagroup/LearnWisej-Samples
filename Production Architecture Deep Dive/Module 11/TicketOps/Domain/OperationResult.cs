...
{
    /// <summary>
    /// What a service hands back to a screen: success or a short, safe explanation.
    /// The screen shows <see cref="Message"/> verbatim, so it must never contain internals.
    /// Unexpected failures are exceptions, not results: the handler catches, logs and shows a generic message.
    /// Module 11 adds one deliberate exception to the "results, not exceptions" rule: an authorization
    /// denial is thrown (<see cref="System.UnauthorizedAccessException"/>) so it can never be mistaken for success.
    /// </summary>
