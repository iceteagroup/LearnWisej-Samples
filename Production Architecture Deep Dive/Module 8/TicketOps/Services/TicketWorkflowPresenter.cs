...
namespace TicketOps.Services
{
    ...
    public sealed class TicketWorkflowPresenter
    {
        ...
        public async Task<WorkflowResult> CloseAsync(int ticketId, string reason)
        {
            ...
            {
                _log.Warn(LogLayer.Service, "TicketWorkflowPresenter.CloseAsync", $"close #{ticketId} denied for {user}");
            ...
            _audit.Record("close", ticketId, user.Id, reason.Trim());
        ...
        public async Task<WorkflowResult> AssignToMeAsync(int ticketId)
        {
            ...
            {
                _log.Warn(LogLayer.Service, "TicketWorkflowPresenter.AssignToMeAsync", $"assign #{ticketId} denied for {user}");
            ...
            _audit.Record("assign", ticketId, user.Id, $"to operator {user.Id}");
