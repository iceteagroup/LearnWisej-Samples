...
The same table is built in code (`ActiveProfile.Entries`).
...
| `INotificationService` | `FakeNotificationService` (records `Sent`) | `EmailNotificationService` (stand-in: logs the SMTP send) | **Transient** | stateless, cheap, one-shot; a fresh instance per resolve cannot leak anything between callers | an outbox that must survive the call — that would need Session or a store |
...
| `ILog` | `ActivityLog` | `ActivityLog` | **Session** | one log per browser session; a Shared log would interleave every user's lines | — |
...
- `AddService<TService, TImpl>(ServiceLifetime.Session)` — contract → type, for classes with a parameterless constructor (`ActivityLog`).
- `AddService<TService>(Func<Type, object> factory, lifetime)` — contract → factory, so the factory can call the implementation's constructor with its collaborators (`new FakePermissionService(users)`); the constructor stays the honest list of what the class needs.
...
- `AddOrReplaceService…` (same three shapes) when a different profile is requested; `HasService<T>()` decides which of the two to call; `GetService<T>()` resolves; `Inject(object)` fills `[Inject]` properties on demand.
...
- Close a ticket in tab 1; tab 2's list still shows it open after **↻ Refresh** (per-session ticket store).
