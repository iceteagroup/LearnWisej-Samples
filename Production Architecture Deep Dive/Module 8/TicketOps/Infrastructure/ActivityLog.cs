...
{
    /// <summary>
    /// Session-scoped implementation of <see cref="ILog"/> (registered with ServiceLifetime.Session) that
    /// writes to the server console. Plain C#: no Wisej.NET dependency, so services can be exercised without a
    /// control. Error details (exception type, message, stack) are written here and never reach the screen.
    /// </summary>
