...
- Other roles who open it see **Access denied** — the page maps the system, so it is for operators only.
...
| | |
|---|---|
| **Trigger** | `/HealthCheck.json` non-200 or wrong `version` for 5 minutes after deploy; **or** the smoke test (`docs/DemoScript.md`) fails; **or** the diagnostics page shows `Unhealthy` with `database → ticket store unreachable` and the store itself is fine |
