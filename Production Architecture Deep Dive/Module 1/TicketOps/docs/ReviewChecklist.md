...
| Controls are named clearly enough for a teammate to follow the event code | ✔ | `gridTickets`, `textTitle`, `comboPriority`, `numericHours`, `buttonSave`, `buttonClose`, `buttonRefresh`; handlers are `<control>_<event>` |
...
| Failure paths are visible, logged, and explained without leaking internals | ✔ | `ShowResult` (expected) and `ReportFailure` (unexpected); exception details go to `ILog` only, the banner shows `Strings.ActionFailed` |
...
| Heavy work synchronously in a handler | no — handlers await the service; Module 7 moves long work to `Application.StartTask` |
