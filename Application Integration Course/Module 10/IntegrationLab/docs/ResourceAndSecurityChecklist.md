...
| 1.4 | A missing/mis-ordered resource fails loudly | ✔ | `heatmap-init.js` → `throw new Error("VendorHeatmap not loaded — check Packages order.")`, reported as one `error {phase:"init"}` event; remove the package and reload the page to see it |
...
| 2.5 | An upgrade has a test page | ✔ | the Operations Dashboard **is** the test page: load, event, both calls, background task, create/dispose test, reload — see [DemoScriptAndTroubleshooting.md](DemoScriptAndTroubleshooting.md) |
...
| 3.4 | Client payloads are never trusted as state | ✔ | `OnWidgetEvent`: day/hour re-validated against the grid (cells outside it are ignored); the .NET event carries the **server** value |
...
| 3.7 | Test-only surfaces do not ship | ✔ | the endpoint answers `load` only; there are no test-only actions or factories. The only diagnostics are the adapter's two counters (`window.__integrationLabCreated/Disposed`) and `getDiagnostics()` |
...
| 4.4 | One flush per meaningful change | ✔ | `live_Updated` changes the heatmap (`Call setCells`), the gauge (`Options.value`) and the tiles inside **one** `Application.Update(page, …)` |
...
| 4.9 | Request budget visible | ✔ | dashboard tile **Requests/min** from `HeatmapWidget.RequestsPerMinute` |
