...
`JsGetSelectedState`) so the two sides can be kept in sync from one place.
...
Use it for every command the server does not need an answer to. The trace shows one line and the
handler never stops:
...
## CallAsync — when the workflow needs the answer
...
visible: the `→` line and the `←` line carry different timestamps.
...
`EvalAsync` is the same mechanism for a JavaScript expression; the sample uses `CallAsync` only.
See `CallAsyncExample.md` for the full awaited example.

## Queueing and batching order

- Every `Call` issued during one request is queued in order and shipped **in one response**.
...
|---|---|
| Is the client result used in the same handler? If not, use `Call`. | `Read rendered size`: yes — the next statement picks a "wide"/"compact" layout and updates `Caption`. `Get selected state`: yes — the DTO is mapped and validated. `Set value` / `Reset animation`: no — so they are `Call`. |
| What happens if the browser never replies (tab closed)? | The `Task` never completes and the handler stays suspended. `SimpleGauge` bounds every awaited call with `ClientReplyTimeout` (5 s, `Task.WhenAny` + `Task.Delay`) and turns "stuck forever" into a `TimeoutException` that the handler shows on the banner. |
| Is the awaited value validated before it is trusted? | Yes. `RenderedSize` must be positive (a hidden element reports 0×0); `GaugeStateDto.Value` must be inside `Minimum..Maximum` or the state is rejected. |

## Evidence

| Button | Trace evidence |
|---|---|
| Set value (72) | one `→ .NET→JS Call("setValue", 72)` line; the needle sweeps to 72 |
| Reset animation | one `→ .NET→JS Call("resetAnimation")` line; the gauge plays its settle animation |
| Read rendered size | `→ await CallAsync("getRenderedSize")`, then `← result {"width":…,"height":…}` on a later timestamp; the caption changes to "Boiler 3 · wide" |
| Get selected state | `→ await CallAsync("getSelectedState")`, then `← result {"value":…,"isAboveThreshold":…,"width":…,"height":…,"isAnimating":…}` |
