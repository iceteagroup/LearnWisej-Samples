...
namespace ValidationClinic
{
    public partial class CustomerIntakeForm : Form
    {
        ...
        private SummaryErrorProvider summaryProvider;
        private readonly Func<List<string>> validateGrid;
        private readonly Func<List<ContactEditModel>> gridRows;

        public event EventHandler Saved;

        public CustomerIntakeForm(ContactRepository repository, Action<string, string> trace, Func<List<string>> validateGrid, Func<List<ContactEditModel>> gridRows)
        {
            this.repository = repository;
            this.trace = trace;
            InitializeComponent();
            SetupValidation();
            this.validateGrid = validateGrid; this.gridRows = gridRows;
        ...
        {
            var messages = fieldMap.Values.Append(lblTitle).Select(errorProvider.GetError).Where(x => !string.IsNullOrEmpty(x)).Concat(unmappedErrors);
        ...
        private void SetError(Control control, string message)
        {
            ...
            summaryProvider.SetError(control, message);
            RefreshSummary();
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            var gridErrors = validateGrid();
            SetError(lblTitle, string.Join(Environment.NewLine, gridErrors));
            if (gridErrors.Count != 0) { Stop("grid"); return; }

            try
            {
                repository.SaveAll(gridRows(), model);
            }
        ...
        {
            summaryProvider = new SummaryErrorProvider(validationSummaryLabel, errorProvider.SetError);
            validation.ErrorProvider = summaryProvider;
