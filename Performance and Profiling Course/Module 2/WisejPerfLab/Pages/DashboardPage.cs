...
namespace WisejPerfLab.Pages
{
    ...
    public partial class DashboardPage : UserControl, IScenarioPage
    {
        ...
        private void RunRefresh(bool fromShell)
        {
            ...
            try
            {
                IReadOnlyList<Data.Entities.Ticket> tickets;
                using (scope.Stage("query + materialise"))
                    tickets = _dashboard.LoadAllTickets();
                ...
                using (scope.Stage("rebuild KPI panel"))
                    RebuildKpiPanel(open, overdue, averageAge);

                using (scope.Stage("rebuild chart"))
