...
namespace OrderDesk.Legacy
{
    ...
    /// automates the Excel that is installed on the user's PC. The COM object model below is the
    /// shape of Microsoft.Office.Interop.Excel (the real reference is not part of the course package;
    /// the stub behaves like Interop does without an interactive desktop). Microsoft does not support
    /// server-side Office Automation — on a web server it hangs or throws unpredictably. Module 6
    /// replaces this with the managed <see cref="OrderDesk.Services.XlsxWriter"/> + Application.Download.
    /// </summary>
    public static class ExcelExport
    {
        public static string ExportToExcel(IEnumerable<Order> orders)
        {
            ...
            try
            {
                ...
                var path = System.IO.Path.Combine(LocalExport.ExportFolder, "out.xlsx");   // ✕ local path again
                wb.SaveAs(path);
                return path;
            }
            finally
...
namespace OrderDesk.Legacy.Excel
{
    ...
    public sealed class Workbook
    {
        ...
        /// <summary>The lab copy never writes outside the project folder (the real one would write to C:\Orders on the server).</summary>
        public void SaveAs(string path) => throw new InvalidOperationException("would write " + path + " on the SERVER");
    }
