...
- `Application.Session` is a `dynamic` bag per session (`Application.Session.Counter = 0;`) **(unverified at runtime)**.
- `Application.Browser` returns `Wisej.Core.ClientBrowser` with `Type`, `Version`, `Device`, `OS`, `UserAgent`, `Language`,
  `ScreenSize`, `TimezoneId`, `IPAddress`, `TabId` **(unverified at runtime; names from the XML docs)**.
- `Application.ApplicationExit` (EventHandler) fires when the session shuts down; `Application.SessionTimeout` fires
  before a timeout; `Application.RunInContext(context, action)` executes without pushing **(unverified at runtime)**.
...
- `Wisej.Web.Timer(components)` with `Interval`, `Tick`, `Start()/Stop()`, `Enabled` — a Component with no visual surface;
  its Tick runs as a normal request, so changes made in Tick are returned with that request, no Update() needed (verified in other courses).
