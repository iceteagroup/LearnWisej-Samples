...
| N-2 | Every control is named for its job with a type prefix — `btn`, `lbl`, `txt`, `cbo`, `dgv`, `lst`, `pnl`, `chk`, `kpi`. **Never `button1`.** | A reviewer reads the handler without opening the Designer. | `btnRefresh`, `dgvIncidents`, `lstTrace`, `kpiSlaAtRisk` |
| N-3 | Handlers are `<control>_<Event>`: `btnRefresh_Click`. Nothing else calls them except another handler that means "then do exactly that". | Grep for a button, find its behaviour. | `btnFeedDown_Click` ends by calling `btnRefresh_Click` — "and then refresh" |
...
| Id | Rule | Why | Example |
|---|---|---|---|
| S-1 | An event handler is **≤ 20 lines** and makes **one** call into a service. The shape is fixed: `try { var result = await _service.XAsync(ctx); ShowResult(result); } catch (Exception ex) { _log.Error(ex); AlertBox.Show(generic); }`. | It is the rule the review gate can check mechanically. | `btnRefresh_Click` — 19 lines |
...
| S-5 | A screen never touches `Data/` or `Integrations/` types outside its constructor. | Otherwise the boundary exists only on paper. | the dashboard's only `_feed` use is the failure-path switch, which *is* the demo |
...
| A-2 | **Expected** failures — validation, policy, stale version — are `Succeeded = false` plus user-safe `Errors`. **Unexpected** failures are exceptions. | The screen can tell "you may not" from "something broke". | `DashboardResult.Denied(...)` vs `IntegrationUnavailableException` |
| A-3 | Security decides first. If the policy denies, nothing else runs — no integration call, no query. | A denial that still hits the database is a leak and a bill. | `RunAsync` step 1, then the trace line `stopped: policy denied — no feed pull, no query` |
...
| A-6 | Every layer writes one trace line for the decision it took, tagged with its layer. | This is how a reviewer proves the boundary without a debugger. | `_trace.Service(…)`, `_trace.Data(…)`, `_trace.Security(…)` |
...
| D-4 | External systems live behind a class in `Integrations/` that can be made to fail on demand. | A failure path you cannot reproduce is a failure path you have never tested. | `OperationsFeed.SetAvailable(false)` |
...
| E-4 | One correlation id per user action, minted at the start of the command and shown on screen. | The trace, the log and the user's screenshot are joinable. | `SessionContext.BeginCommand()` → `lblCorrelation` |
...
| T-2 | Every ADR has a review date; the sweep is a calendar item, and `DecisionLog` makes the dates queryable from the app. |
