...
| `RunTrustedClientCommand(commandName, entityId, claimedRole, claimedStatus)` | `CommandCenterShell`, automatic | `App.MainPage.RunTrustedClientCommandAsync(…)` | `ClientCommandResult` ⚠ anti-pattern |
...
4. **Never leak.** `catch (Exception)` answers `SERVER_ERROR` with a user-readable message. The
   exception type goes to the trace; the message, the stack and the internal ids do not go to the browser.
...
Every server → client call on `CommandPaletteHost` goes through one private method, `Send(label, call)`:

```csharp
if (!PaletteReady || !this.IsLoaded) { DeferredCalls++; _pending.Add(call); Deferred?.Invoke(…); return; }
...
| `RunFromClient(name, entityId)` | `paletteRun` | make the round trip really start in the browser |
| `RunWithBadCorrelation(name, entityId)` | `paletteRunRaw` | send a payload that breaks the shape |
| `RunTrustedFromClient(name, entityId, role, status)` | `paletteRunTrusted` | ⚠ drive the anti-pattern endpoint |
| `CollectCapabilities()` | `paletteCollect` | re-run feature detection |
| `SendForgedCapabilities()` | `paletteForge` | send a report with keys the server never published |
...
| --- | --- | --- |
| user | — (the browser is not told who it is beyond the header) | `SessionContext.UserName` |
...
| --- | --- | --- |
| Automatic page registration | **Run approve** | trace `Client: App.MainPage.RunClientCommand(…)` |
| `RegisterWebMethods` on a child | press Ctrl+K and type | trace `Interop: GetCommandCatalog(query="app") → 1 of 6 commands (display filter only)`; the palette list narrows |
| Deferred callback | page load | trace `… DEFERRED — the host widget does not exist yet`, then `flushing 1 deferred server → client call(s)`; the bottom bar shows `host: ready · 1 deferred` |
| Failure isolation | **Malformed payload** | the Promise still resolves, the palette footer shows `MALFORMED_PAYLOAD`, and no exception popup appears |
| Both directions audited | any command, then read the audit card | one row per crossing: outcome · user · tenant · correlation · command · entity |
