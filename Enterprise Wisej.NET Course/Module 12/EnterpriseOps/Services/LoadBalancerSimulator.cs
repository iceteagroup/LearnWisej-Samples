...
namespace EnterpriseOps.Services
{
    ...
    /// app-node-B is a peer: the same checks run, and its database check reflects whether the release's
    /// migration completed on it.
    ///
    /// A Wisej.NET session lives in ONE node's memory, so the balancer pins it (sticky sessions / affinity)
    /// and only steers NEW sessions away from a node that fails readiness.
    /// </summary>
    public class LoadBalancerSimulator
    {
        ...
        public void RefreshHealth()
        {
            foreach (var node in Nodes)
            {
                ...
                // Log only changes: a probe that keeps saying "Healthy" every ten seconds is not news.
