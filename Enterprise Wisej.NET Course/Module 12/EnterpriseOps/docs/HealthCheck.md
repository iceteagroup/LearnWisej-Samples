...
3. The server log records the balancer's decision: `Balancer: app-node-B failed readiness (…) — routed away`.
...
- When a node is unhealthy, the page's dark line under the node grid is its probe body.
- The first **Deploy…** of 2.4.2 hits the migration that does not complete on node B, and the probe answers
  `app-node-B { "status": "Unhealthy", … database: FAIL · storage: OK · websocket: OK }`.
