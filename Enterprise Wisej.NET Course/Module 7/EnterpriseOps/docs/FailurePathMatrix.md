...
| **notify** | receipt id in the trace, `NotificationStatus.Sent` | n/a | ▶ SMTP timeout is the external-failure cell (a timeout tells you nothing about delivery) | n/a — the channels were validated in step 5 | ▶ **cannot** be undone: the escalation is **kept**, `NotificationStatus.ManualReview`, `NotificationOutstanding` compensation queued, `CreatedWithCompensation` + `CompensationAction: manual-review queued (#n)`; audited as "approver NOT notified" |
...
| `AuditGap` | ▶ **Resolve manually** (a retry is meaningless — the message is out) | entry `Resolved` with who closed it and why |
...
| **Fail: directory timeout** → Escalate → step 3 | A · 3 · timeout |
...
| **Fail: notification (SMTP)** → Finish | B · notify · external |
| **Fail: audit after notify** → Finish | B · audit · external |
| **Retry notification** on the queued entry | C · NotificationOutstanding |
| **Resolve manually** on an audit gap | C · AuditGap |
| **Anti-pattern: logic in the page** | the counter-example: the same failure with no matrix behind it |
