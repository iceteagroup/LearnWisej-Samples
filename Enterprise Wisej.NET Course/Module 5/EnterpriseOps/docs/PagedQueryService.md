...
does that on every filter change, every sort change and every page click. The server log shows it on each search:
...
If the answer were "all of them", the design would not have started. `PerformanceNotes.md` measures exactly that
alternative.
...
|---|---|---|
| 1 · tenant guard | `SearchAsync` | Rejects a query whose `TenantId` is not the session's, whatever the UI sent. Logged as `Security:`. |
...
selection live in `Services/WorkQueues/GridState.cs`, owned by `SessionContext` — not by `WorkQueuePage` — so a
rebuilt page reads the same grid state back. `LastElapsedMs` and `LastPayloadBytes` on the service are measurements
of the last call, used by the status bar and by the notes.
...
1. A query is always scoped to `SessionContext.TenantId`. A query naming another tenant throws
   `UnauthorizedAccessException` and is logged — it never returns an empty result that looks like "no data".
2. `PageSize` is clamped. A client asking for `PageSize = 1000000` gets 200.

## Evidence in the running app

| Do this | The server log shows |
|---|---|
