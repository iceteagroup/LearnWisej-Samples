...
| B4 | Pending work is countable at a glance | ✅ queue title: `COMPLETION QUEUE — 2 PENDING` |
...
| B7 | A failed sync is recoverable without losing anything | ✅ the queue is untouched on failure; the next reconnect retries |
...
| D5 | Battery: no polling loop while idle | ✅ no `Timer`; the replay runs only when the technician reconnects |
| D6 | A wiped or doubted device recovers by re-provisioning | ⚠️ partial — `LocalStore.Wipe()` and `ProvisionDeviceAsync()` exist, but the prototype has no re-provisioning action |

## E. Security, from the technician's point of view

| # | Check | Sample |
|---|---|---|
| E1 | Local data is encrypted at rest | ✅ modelled (`LocalStore.EncryptedAtRest`), logged on every cache download |
...
| E4 | A revoked user's queued work is rejected, not applied | ✅ `WorkOrderService.Complete` re-checks `workorder.complete` on the server for every replayed command; a refusal ends `Rejected` with an audit entry |
| E5 | The technician sees why something was refused | ✅ the server's message reaches the status strip, the toast and the card |

## Scoring the checklist

25 checks: **20 pass**, **5 partial** (A6 high-contrast theme, D1 keyboard-free entry, D4 durable store,
D6 re-provisioning action, plus C-series items that depend on the production `LocalStore.Sqlite`). The partials are all
...
Walk the checklist with the app open on a phone or in the browser's device emulation (A1, A5), press
**Go offline** (B1, B2), complete two work orders including WO-1037 (B3, C1), press **Go online** (B5, B6),
and resolve the conflict (C2, C3, E5).
