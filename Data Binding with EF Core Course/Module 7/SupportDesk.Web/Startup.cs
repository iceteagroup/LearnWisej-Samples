...
//    EnableSensitiveDataLogging is switched on only when isDevelopment is true.
...
Console.Error.WriteLine($"[SupportDesk] environment: {app.Environment.EnvironmentName} · sensitive-data logging: {(app.Environment.IsDevelopment() ? "ON (Development)" : "OFF")}");

// 5. Development convenience: migrate the local SQLite file and seed it before the first session starts.
//    Production applies the reviewed script (artifacts/sql/supportdesk_migrations.sql) as a release step
//    instead, so Migrate() never runs outside Development. See docs/DeploymentNotes.md.
if (app.Environment.IsDevelopment())
    await SupportDeskDevelopmentDatabase.EnsureReadyAsync(app.Services);
else
    Console.Error.WriteLine("[SupportDesk] migrations are not applied at startup outside Development: apply artifacts/sql/supportdesk_migrations.sql as a reviewed release step.");
