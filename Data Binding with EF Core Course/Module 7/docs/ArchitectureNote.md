...
|---|---|---|
| Session state | Wisej.NET | `TicketBrowserPage` — created once per browser session (`Program.Main`), lives as long as the tab. Holds `ticketBindingSource` (the `List<TicketListItem>` the last search returned), the current page index, `cboRole`'s selection, and the trace list. |
...
SupportDesk.Services (stateless, UI-free — no reference to Wisej.Web)
    ...
    TicketCommandService      write-side: load-for-edit, save, delete, close-with-comment, simulate-change
    ...
    │  holds only IDbContextFactory<SupportDeskContext> (+ small lab-prop singletons)
    ▼
SupportDesk.Data (EF Core — the only project referencing the SQLite provider)
    SupportDeskContext, Entities/, Migrations/, Diagnostics/ (QueryTrace, DevelopmentOutageSwitch,
    TransactionFailureSwitch, StartupDiagnostics)
```

Every service in `SupportDesk.Services` is **Transient** and holds only the factory (plus, where the lab
needs one, a small singleton lab prop like `TransactionFailureSwitch`) — never a `DbContext`, never mutable
session state of its own. `TicketValidator` and `ConflictResolution` do not even reference
`Microsoft.EntityFrameworkCore.Sqlite` or `Wisej.Web` — they are UI-free and database-free by construction,
which is what makes `TicketValidatorTests` and the `ConflictResolution` tests in
...
| Method | Purpose |
|---|---|
| `TicketQueryService.SearchTicketsAsync` / `…SlowlyAsync` | one context, two statements (COUNT + paged SELECT) |
| `TicketQueryService.CountTicketsAsync` / `…SlowlyAsync`, `GetStatusesAsync`, `GetCustomersAsync` | one context each |
...
| `TicketCommandService.SimulateAnotherOperatorChangeAsync` | one context — deliberately separate from whatever context an open editor is using, to reproduce a real second-operator write |
| `TicketCommandService.CloseTicketWithCommentAsync` | **one context, two `SaveChangesAsync` calls, one explicit transaction** spanning both (see `docs/TransactionsAndRetries.md`) |
...
- `Startup.cs` — the bridge line and the service registrations (all `AddTransient`, plus the singletons
  registered inside `AddSupportDeskData`).
- Every method named above, read directly — each opens with `await using var db = await
  _dbFactory.CreateDbContextAsync(...)` and the `SupportDeskContext` constructor/`Dispose` pair reports to
  `QueryTrace`, which is what lets the page's trace list show `◦ context #n created` / `◦ context #n
  disposed` for every one of them.
