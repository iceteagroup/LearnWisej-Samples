...
/// </para>
/// <para>
/// <b>RowVersion — Module 7.</b> <see cref="RowVersion"/> is hidden state: no control binds to it, and
/// <see cref="TicketValidator"/> never looks at it (it is not a business rule, it is plumbing). It is
/// filled in <c>TicketCommandService.LoadEditModelAsync</c> from the no-tracking read, rides along with
/// every other field while the operator edits, and — for an existing ticket — <c>TicketCommandService.SaveAsync</c>
/// restores it onto the freshly loaded tracked entity with
/// <c>db.Entry(ticket).Property(t => t.RowVersion).OriginalValue = model.RowVersion</c> <b>before</b> mapping
/// the other fields. That one line is what turns a stale edit into a conflict: without it, the fresh read
/// inside <c>SaveAsync</c> always compares the token against itself (the version it just loaded is always
/// "current"), and <c>DbUpdateConcurrencyException</c> is never thrown — the bug the Module 7 lab's first
/// student review question asks about. See <c>docs/ConcurrencyResolution.md</c> for the full round trip,
/// the conflict dialog and the two-session reproduction.
/// </para>
/// <para>
...
public sealed class TicketEditModel : INotifyPropertyChanged
{
    ...
    /// <summary>
    /// Module 7's concurrency token: the <c>RowVersion</c> the editor read when it loaded this ticket.
    /// Hidden state — no control binds to it, nothing paints it, <see cref="TicketValidator"/> ignores it —
    /// but it must travel with the model from <c>LoadEditModelAsync</c> to <c>SaveAsync</c> so the fresh
    /// context used to save can restore it as the tracked entity's <c>OriginalValue</c>. Empty for a new
    /// ticket (<see cref="Id"/> == 0): there is nothing to compare against yet.
    /// </summary>
