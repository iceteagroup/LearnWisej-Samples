...
public sealed class TicketCommandService
{
    ...
    /// <c>OriginalValue</c>. That is the whole difference between an ordinary save and an overwrite: the
    /// UPDATE now carries the version the database holds right now (which the operator has chosen to
    ...
    public async Task<SaveTicketResult> SaveAsync(TicketEditModel model, TimeSpan latency, bool forceDuplicateNumber, byte[]? overwriteOriginalRowVersion, CancellationToken token = default)
    {
        ...
        else
        {
            ...
            // The RowVersion round trip (Module 7). The token the operator's edit is based on — the fresher
            // database token on Overwrite, otherwise the one TicketEditModel carried from LoadEditModelAsync —
            // becomes the tracked entity's OriginalValue BEFORE any field is mapped, so EF Core's
            // UPDATE … WHERE "Id" = @p AND "RowVersion" = @original compares against what the operator
            // actually saw (or agreed to discard), not against the value this fresh read just fetched (which
            // is always "current" and would make a conflict impossible to detect).
            ...
            if (originalToken is { Length: > 0 })
            {
                ...
                QueryTrace.Note($"SaveAsync: existing ticket {ticket.Number} (#{ticket.Id}) — tracked read, fields mapped from the edit model · OriginalValue RowVersion = 0x{Convert.ToHexString(originalToken)}" + (overwriteOriginalRowVersion != null ? " (OVERWRITE: forced to the database's current token)" : ""));
            }
            else
            {
                QueryTrace.Note($"SaveAsync: existing ticket {ticket.Number} (#{ticket.Id}) — tracked read, fields mapped from the edit model (no RowVersion on the model — OriginalValue left as read)");
            }
        }
        ...
        {
            // ex.Entries belong to `db`, which is still open here — GetDatabaseValuesAsync needs that. The
            // `await using` above disposes `db` as soon as this method's stack frame unwinds, which happens
            // BEFORE the exception reaches the editor's own catch block, so the conflict list must be built
            // now, while the context is alive, and carried on the exception itself (see ConflictResolution's
            // remarks) rather than rebuilt later from entries that would already be unusable.
            var conflicts = await _conflictResolution.BuildConflictListAsync(ex, token);
            QueryTrace.Note($"SaveAsync: caught DbUpdateConcurrencyException — 0 rows matched the WHERE clause · {conflicts.Fields.Count} field(s) differ" + (conflicts.DeletedByAnotherUser ? " · the row was deleted by another user" : ""));
    ...
    private static TicketEditModel ToModel(Ticket ticket) => new()
    {
        ...
        // Module 7: the concurrency token this no-tracking read saw, carried as hidden state so SaveAsync
        // can restore it as OriginalValue on the tracked entity it loads later — see TicketEditModel's remarks.
