...
database failure reaches `TicketEditorForm`'s `catch` blocks (`DbUpdateException`, then `Exception`),
which show one friendly sentence in an error `AlertBox` and write the full exception to the server
console.

## Reproducing "already deleted" with two browser tabs

Two tabs are two Wisej.NET sessions, which is exactly the multi-user situation the handling is for:

1. Open the ticket browser in tab A and tab B.
2. In tab A, select a ticket that is not `Closed`, **Edit Ticket** → **Delete** → **Yes**. Tab A's grid
   refreshes; tab B's grid still shows the row.
3. In tab B, select the stale row and click **Edit Ticket**. `LoadEditModelAsync` throws
   `TicketNotFoundException`; the editor shows *This ticket was already deleted by someone else. The list
   will refresh.* and closes with `DialogResult.OK`, so tab B's grid refreshes too.

If tab B already had the editor open on that ticket before step 2, **Save** shows *This ticket was already
deleted by someone else. Nothing was saved.* and **Delete** shows the `NotFound` reason; both close with
`DialogResult.OK`.
...
- `DeleteAsync_returns_NotFound_and_sends_only_one_statement_when_the_ticket_is_already_gone` — a second
  `DeleteAsync` on the same id returns `DeleteOutcome.NotFound` with a friendly `Reason`, and the test's
  command count shows **exactly one statement** (the `SELECT`) — no `DELETE` is ever sent for a row that is
  not there.
...
- `SaveAsync_throws_TicketNotFoundException_when_the_ticket_was_deleted_since_it_was_loaded` — loading the
  edit model, then deleting the row through the service (as a second session would), then saving the stale
  model throws `TicketNotFoundException` carrying the ticket's id — the exception
...
`No` (or dismissing the dialog) truly sends nothing; whether the top-right `AlertBox` renders for the
`Refused` and `NotFound` cases; whether tab B's grid really shows the stale row until the next search, and
loses it once Edit on it closes with `DialogResult.OK`.
