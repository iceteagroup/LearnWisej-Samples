...
public sealed class DevelopmentSeederTests : IDisposable
{
    ...
    [Fact]
    public async Task Seed_inserts_five_customers_three_agents_six_categories_and_at_least_fifty_tickets()
    {
        ...
        Assert.True(result.Tickets >= 50, $"expected at least 50 tickets, got {result.Tickets}");
    ...
    public async Task Seed_data_is_realistic_and_mixed()
    {
        ...
        Assert.Equal(new[] { "Closed", "In Progress", "Open", "Waiting" }, tickets.Select(t => t.Status).Distinct().OrderBy(s => s));
