...
{
    /// <summary>
    /// Module 4 lab page — the Support Desk ticket browser gains Add and Edit, on top of Module 3's search
    /// and paging, Module 2's model and Module 1's first query.
    ///
    ...
    ///             statusLabel, prevPageButton / nextPageButton, and now CellDoubleClick opens the editor)
    ...
    ///             <see cref="AddTraceRaw"/> callback the dialog is given when it is opened.
    /// Under both: the Module 2 "Model &amp; migration" card and the Module 1 four-lifetimes table.
    /// Bottom bars: row 1 = the Module 4 paths (Add, Edit, the slow-save toggle, "simulate another
    ///             operator deletes it"); row 2 = the Module 3 paths (slow search, outage, recovery, the
    ///             bound-IQueryable anti-pattern — <c>buttonBreak</c>/<c>buttonRestore</c> also govern the
    ///             editor's Save and Delete, since both go through the same <see cref="Outage"/> switch);
    ///             row 3 = the Module 2 paths; row 4 = the Module 1 paths.
    ///
