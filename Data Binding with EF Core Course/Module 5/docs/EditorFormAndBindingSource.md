...
Save always wins, on purpose (see `docs/LoadAndSaveFlows.md`). This is a deliberate scope cut, noted in
`TicketEditModel`'s XML doc comment and in the lab guide's lesson text (which mentions RowVersion as part of
a *later* concurrency check the reading discusses but this module does not implement).
...
shown with `new TicketEditorForm(...)` the same as it does to `TicketBrowserPage`. (Module 6 also injects
`TicketDetailService` for the comments panel.)
...
`btnDelete`, `btnCancel`, `btnSave`. Module 6 adds `lstComments` and `btnShowFullHistory`.
