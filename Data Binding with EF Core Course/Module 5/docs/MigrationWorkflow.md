...
Run from the **module folder** (`Module 3/`). `--framework net10.0` is required because
...
schema it can only read. The development start makes the difference visible: the console line reports
how many migrations were pending and applied, and `dotnet ef migrations list` marks anything a database
has not applied with `(Pending)`, so a pending migration shows up instead of being applied behind the
operator's back. (Module 2's page also showed a *Pending migrations* count; from Module 3 on the page is
the ticket browser.)
...
`nothing to seed: 312 tickets already exist`.
