...
// 3. Application services. They hold only the factory, so any lifetime is safe; Scoped follows the course text.
builder.Services.AddScoped<TicketQueryService>();
builder.Services.AddScoped<SharedContextAntiPattern>();
...
// 5. Development convenience: make sure the local SQLite file exists before the first session starts.
