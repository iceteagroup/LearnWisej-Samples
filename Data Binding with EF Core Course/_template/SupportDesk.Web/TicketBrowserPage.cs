    ...
    public partial class TicketBrowserPage : Page
    {
        ...
        private void UpdateLifetimes()
        {
            ...
            this.labelLifetimes.Text =
                ...
                Row("lifetime", "owner", "this session, right now") +
                Row("session state", "Wisej.NET", "id " + ShortSessionId() + " · started " + _createdAt.ToString("HH:mm:ss")) +
                Row("UI object", "this Page", "1 page, its labels and this trace list — alive until the tab closes") +
                Row("request / thread", "one click", _handlerRuns + " handler run(s) · after an await the continuation may resume on any thread") +
