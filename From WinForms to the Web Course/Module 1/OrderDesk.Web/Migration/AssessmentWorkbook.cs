...
namespace OrderDesk.Migration
{
    ...
    public static class AssessmentWorkbook
    {
        public static readonly IReadOnlyList<AssessmentItem> Items = new List<AssessmentItem>
        {
            ...
            new AssessmentItem { Area = "Forms & controls", Feature = "Edit Order dialog", Dependency = "modal Form · DialogResult", RiskTag = "direct-port", Verdict = Verdict.DirectPort, Effort = "S", InFirstSlice = true,
                Why = "Ordinary handlers and a modal result — the event-driven model comes across.",
                Replacement = "None needed; add disposal (using block) and a Toast instead of MessageBox (Module 3)." },
            ...
            new AssessmentItem { Area = "Data access", Feature = "Connection string", Dependency = "App.config", RiskTag = "startup", Verdict = Verdict.Adapt, Effort = "S", InFirstSlice = true,
                Why = "App.config is gone; configuration belongs to the web host.",
                Replacement = "Web.config / appsettings (Module 2)." },
            new AssessmentItem { Area = "Forms & controls", Feature = "Menu bar (File · View · Reports · Help)", Dependency = "MenuStrip", RiskTag = "direct-port", Verdict = Verdict.DirectPort, Effort = "S", InFirstSlice = true,
