...
* **Modal dialogs do not block** — `EditOrderDialog` ported with `ShowDialog(callback)` / `ShowDialogAsync`; the caller
  disposes the dialog when it closes (leak counter proves it). `MessageBox` → `Toast` for confirmations,
  `MessageBox.ShowAsync` for decisions.
* **Layout** — fixed coordinates vs `Dock`/`Anchor` reviewed; blocking work → `Application.StartTask`.

## Module 4 · Sessions, statics and multi-user safety (2026-09-10)

* **Static-state audit** — every writable static listed; per-user state moved into `UserSessionContext` behind
  `Application.Session`; the two-session corruption test (legacy statics vs context) reproduced and fixed.
* **Registry** — HKCU → per-user profile store on the server (`App_Data/profiles`) + browser `localStorage` for UI prefs.
* **Timeout & logout** — `Application.SessionTimeout` / `ApplicationExit` handled; per-session temp cleaned up.

## Module 5 · DataGridView, validation and performance (2026-09-10)

* **200,000 orders** — naive load-all vs server-side filter + `VirtualMode` block cache (`OrderQuery.Skip/Take`);
  timing table recorded; summary row; `OrderValidator` → `ValidationResult` shown through `ErrorProvider`.
...
* **Theme + mixin** — `Default.json` theme `Bootstrap-4`; `ThemeService.Apply(name)` = `Application.LoadTheme` restyles
  **every session in the process** (a deployment decision, not a user preference); `Themes/orderdesk.mixin.theme`
  (button + toolbar-button radius 14) merged at startup and re-applied explicitly with `LoadTheme(current, new[] {"orderdesk"})`;
  the merged value is read back with `Application.Theme.GetStyle<object>("button", "radius", "default")`.
* **Responsive** — `ClientProfiles.json` (Phone ≤600 · Tablet 601–1024 · Desktop ≥1025, copied next to the assembly,
  ...
  `Application.ResponsiveProfileChanged` subscribed in `Load`, unsubscribed in `Dispose`; simulate buttons call the same
  `Apply`. Only what `docs/ResponsiveProfiles.md` lists is claimed — no phone device was used.
* **Security as a web app** — `AuthService.SignIn` verifies salted SHA-256 hashes (constant-time compare) and stores
  ...
  `orders.export`; sam the Clerk is denied although the button is clickable). `DownloadGuard`: permission → resolve
  under the storage root (rooted paths and `..` refused) → exists → `Application.Download(resolvedPath)`;
  `..\Web.config` is rejected and audited. `docs/SecurityReview.md`.
* **AllowHtml policy** — `Label.Text` is encoded by default (`AllowHtml = false`); `AllowHtml = true` only for trusted
  markup or output of `HtmlSanitizer.Sanitize` (whitelist `b`, `i`, `br`, no attributes). The console renders the same
  user text three ways; the raw one shows `<img src=x onerror="alert(1)">` executing — the biggest injection risk in
  a migrated app is a property flip on a control that used to be inert.
* **Audit log** — `AuditLog` (process-wide, append-only, locked, `Version` counter): sign-in/out, authorize denials
  (recorded inside `Authorize`, so no caller can forget), export, download, print, theme; mirrored to
  `App_Data/logs/orderdesk-yyyyMMdd.log` by `AppLog` (+ `System.Diagnostics.Trace`). A 1 s `Wisej.Web.Timer`
  redraws grids only when `Version` or `SessionCount` changed — the second session's actions appear in the first tab
  without any push from it.
* **Operations dashboard** — KPIs from the orders (Open, Revenue today, Invoiced, On-time % = not on hold),
  ...
  `● N sessions · M audit entries` live line.
* **Health & readiness** — `/health` (`Startup.cs` `MapGet`, no Wisej session; `HealthCheck.Json()`: status, version,
  uptime, storage root writable, log folder, `ClientProfiles.json`, mixin, `ok`); in-session report adds sessions,
  theme, profile, web socket. `ReadinessChecklist` = the lesson's 8 items as code with live evidence;
  `StaticStateAudit` reflects over every static field (0 open items; `AuditLog._version` is the accepted counter).
...
* **Build** — both target frameworks (`net10.0`, `net10.0-windows`) build with 0 warnings / 0 errors; the
  `HealthCheck` name clashes with `Wisej.Core.HealthCheck` once `Wisej.Core` is imported for `ClientProfile` — resolved
  with a `using HealthCheck = OrderDesk.Diagnostics.HealthCheck;` alias.
