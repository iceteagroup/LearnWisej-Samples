...
* **Import** — `File.ReadAllLines(@"C:\Orders\in.csv")` → `Wisej.Web.Upload` (`.csv`, ≤ 1 MB) → `uploads/` →
  ...
  layouts accepted (line layout; the Module 1 export layout). Bad rows are skipped with a reason. The legacy read
  would throw `DirectoryNotFoundException` (Windows) / `FileNotFoundException` (Linux) on the server.
...
* **Office Automation removed from the migrated path** — the desktop `ExcelExport` is not carried over; automating
  Excel from a server process is unsupported (KB 257757) and impossible on Linux.
...
* **Server-like test plan** — `docs/ServerLikeTest.md`: no-Office pass, non-admin service account, Linux container;
  both target frameworks build with 0 errors on this machine; the Linux run was not executed here.
