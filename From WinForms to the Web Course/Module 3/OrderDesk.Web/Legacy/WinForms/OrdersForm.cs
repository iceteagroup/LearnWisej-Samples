...
namespace LegacyOrderDesk
{
    ...
    public partial class OrdersForm : Form
    {
        ...
        private void ordersGrid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dialog.ShowDialog(this) == DialogResult.OK)   // ✕ blocking ShowDialog: on the server the call returns immediately — the result arrives in a callback
            {
                _orderService.Save(dialog.Order);                 // ✓ business logic reused
                MessageBox.Show("Saved.", "LegacyOrderDesk");      // ✕ a modal with nothing to decide: one extra click, the whole page masked → Toast (Module 3)
                ReloadGrid();
            }
        }
        ...
        private void settingsMenuItem_Click(object sender, EventArgs e)
        {
            using (var dialog = new SettingsForm())
                dialog.ShowDialog(this);   // ✕ a modal Settings dialog → a screen in the content host (Screens/SettingsScreen)
        }
