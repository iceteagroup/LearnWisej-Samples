...
{
    /// <summary>
    /// The service layer the lesson keeps business logic in. The Tickets page, the Dashboard commands and the
    /// dialog never touch the list directly: they call AddTicket / UpdateTicket / DeleteTicket / SaveTicket.
    ///
    /// Module 7 does NOT change this class. The theme selector, the cards, the navigation highlight and the
    /// spacing rules all live in the UI layer; nothing in here knows that a theme exists.
    ///
    /// The list is an instance field, so each user session (each MainWindow) owns its own data —
